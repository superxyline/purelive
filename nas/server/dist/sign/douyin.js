...
exports.evalDouyinMsStub = evalDouyinMsStub;
exports.evalDouyinSignature = evalDouyinSignature;
exports.generateMsToken = generateMsToken;
exports.evalDouyinAbogus = evalDouyinAbogus;
exports.evalDouyinWebMsSignature = evalDouyinWebMsSignature;
/**
...
const K_ABOGUS_JS = `function getABogus(params, userAgent) {
...
const K_WEB_MS_SDK_JS = `function getMSSDKSignature(msStub, userAgent) {
    document = {}
    window = {}
    navigator = {
        userAgent: userAgent
...
const K_DOUYIN_DEFAULT_UA = 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.5845.97 Safari/537.36 Core/1.116.567.400 QQBrowser/19.7.6764.400';
...
function encodeXbogusBase64(data) {
    let out = '';
...
function md5Last2(hexStr) {
    ...
    const digest = (0, crypto_1.createHash)('md5').update(Uint8Array.from(bytes)).digest();
    return [digest[14], digest[15]];
}
/** 生成 X-Bogus（xbogus.dart generateXBogus；msStub 为 32 位 md5 hex）。 */
function generateXBogus(msStub, counter = 1) {
    if (msStub.length !== 32) {
    ...
    const finalData = new Array(12).fill(0);
    finalData[0] = header;
    finalData[1] = random2;
    for (let i = 0; i < 10; i++)
...
function buildWssSigParam(roomId, uniqueId) {
    const params = {
        live_id: '1',
        aid: '6383',
        version_code: '180800',
...
function evalDouyinMsStub(roomId, uniqueId) {
    const params = [
        ['live_id', '1'],
        ['aid', '6383'],
        ['version_code', '180800'],
    ...
    return (0, crypto_1.createHash)('md5').update(sigParams, 'utf8').digest('hex');
}
/**
 * 弹幕 wss URL 的 signature 参数。
 * 对应 Dart：DouyinDanmaku.getSignature —— generateXBogus(md5(参数串), 1)。
 */
function evalDouyinSignature(roomId, uniqueId) {
    const md5SigParam = (0, crypto_1.createHash)('md5').update(buildWssSigParam(roomId, uniqueId), 'utf8').digest('hex');
 ...
 */
function evalDouyinAbogus(url, userAgent) {
    ...
    vm_1.default.runInContext(K_ABOGUS_JS, sandbox, { timeout: 10_000 });
    const aBogus = vm_1.default.runInContext(`getABogus(${JSON.stringify(query)}, ${JSON.stringify(userAgent)})`, sandbox, { timeout: 10_000 });
 ...
 * 返回值含 -/= 时重试（Dart 同款循环）。
 */
function evalDouyinWebMsSignature(msStub, userAgent) {
    const sandbox = vmSandbox();
    vm_1.default.createContext(sandbox);
    vm_1.default.runInContext(K_WEB_MS_SDK_JS, sandbox, { timeout: 20_000 });
    const call = () => String(vm_1.default.runInContext(`getMSSDKSignature(${JSON.stringify(msStub)}, ${JSON.stringify(userAgent)})`, sandbox, { timeout: 20_000 }));
    let signature = call();
    let guard = 0;
