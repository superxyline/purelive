...
function getUserId(): number {
  const match = /DedeUserID=(\d+)/i.exec(getCookie('bilibili'));
  return match ? Number(match[1]) : 0;
}

function getBuvidFromCookie(): { b_3: string; b_4: string } {
...
async function getBuvid(): Promise<{ b_3: string; b_4: string }> {
  try {
    const cookie = getCookie('bilibili');
    if (cookie.includes('buvid3')) {
      return getBuvidFromCookie();
    }
    const result = await upstreamJson('https://api.bilibili.com/x/frontend/finger/spi', {
...
async function getJson(
  ...
  return upstreamJson(query ? `${url}?${query}` : url, { headers: header });
}
...
async function getWbiKeys(forceRefresh = false): Promise<[string, string]> {
...
async function getWbiSign(url: string, forceRefresh = false): Promise<Record<string, string>> {
  const [imgKey, subKey] = await getWbiKeys(forceRefresh);
  const mixinKey = getMixinKey(imgKey + subKey);
  const currentTime = Math.floor(Date.now() / 1000);

  const queryParams: Record<string, string> = {};
  for (const [k, v] of new URL(url).searchParams) queryParams[k] = v;
...
function parseFollowersFromRoomInfo(roomInfo: any): string {
...
async function fetchFollowersFromRoomPage(realRoomId: string): Promise<string | null> {
  if (realRoomId === '') return null;
  try {
    const res = await upstream(`https://live.bilibili.com/${realRoomId}`, { headers: await getHeader() });
    const html = res.text;
...
async function discoverDanmaku(realRoomId: number, maxAttempts = 4): Promise<Record<string, unknown>> {
  ...
  if (!data) throw new Error(`Bilibili danmaku discovery failed: ${String(lastError)}`);
  ...
  const serverUrls: string[] = [officialFallback];
  for (const item of (data['host_list'] as any[]) ?? []) {
  ...
  const uid = getCookie('bilibili').trim() === '' ? 0 : getUserId();
  const danmakuHeaders: Record<string, string> = {
    'user-agent': headers['user-agent'] ?? K_USER_AGENT,
    origin: 'https://live.bilibili.com',
    referer: `https://live.bilibili.com/${realRoomId}`,
  };
  if ((headers['cookie'] ?? '') !== '') danmakuHeaders['cookie'] = headers['cookie'];
  ...
  return {
    ...
    serverUrls,
    buvid: buvid3,
    cookie: headers['cookie'] ?? getCookie('bilibili'),
    headers: danmakuHeaders,
  };
}

/** 未完成握手时的默认弹幕参数（进入房间后由弹幕层刷新凭据）。 */
function defaultDanmakuArgs(realRoomId: number): Record<string, unknown> {
  const headers: Record<string, string> = {
    'user-agent': K_USER_AGENT,
    origin: 'https://live.bilibili.com',
    referer: `https://live.bilibili.com/${realRoomId}`,
  };
  const cookie = getCookie('bilibili').trim() === '' ? `buvid3=${buvid3};buvid4=${buvid4};` : getCookie('bilibili');
  if (cookie !== '') headers['cookie'] = cookie;
  return {
    roomId: realRoomId,
    uid: getCookie('bilibili').trim() === '' ? 0 : getUserId(),
    token: '',
...
function normalizeNetworkImageUrl(source: unknown): string {
  ...
  try {
    ...
    if ((uri.protocol === 'http:' || uri.protocol === 'https:') && uri.hostname !== '') return value;
  } catch {
    // not absolute
  }
...
function parseRecommendRooms(response: any): LiveRoomJson[] {
  if (!response || typeof response !== 'object') throw new Error('Bilibili response is not an object');
  if (response['code'] !== 0) {
  ...
  if (!Array.isArray(rawList)) throw new Error('Bilibili recommendation list is missing');
...
async function getAccessId(): Promise<string> {
  ...
  const res = await upstream('https://live.bilibili.com/lol', { headers: await getHeader() });
  const id = /"access_id":"(.*?)"/.exec(res.text)?.[1]?.replaceAll('\\', '') ?? '';
  accessId = id;
  return accessId;
}

function buildSearchUrl(searchType: string): string {
...
class BiliBiliSite implements Site {
  ...
  async getCategoryRooms(cateId: string, typeName: string, page: number, _pageSize: number): Promise<LiveRoomJson[]> {
    ...
    try {
      ...
      if (found) areaType = found.areaType;
    } catch {
    ...
    for (const item of result['data']?.['list'] ?? []) {
      items.push({
        ...emptyRoom('bilibili', String(item['roomid'] ?? '')),
        title: String(item['title'] ?? ''),
  ...
  async getRecommendRooms(page: number, pageSize: number): Promise<LiveRoomJson[]> {
    ...
    try {
      ...
      throw new Error(
        `Bilibili recommend failed: primary=${String(primaryError)}; fallback=${String(fallbackError)}`,
      );
    }
  }
  ...
  async searchRooms(keyword: string, page: number, _pageSize: number): Promise<LiveRoomJson[]> {
    const result = await getJson(
      buildSearchUrl('live'),
      { ...SEARCH_COMMON_PARAMS, keyword, page },
      await getHeader(),
    );
  ...
  async searchAnchors(keyword: string, page: number, _pageSize: number): Promise<LiveAnchorItemJson[]> {
    const result = await getJson(
      buildSearchUrl('live_user'),
      { ...SEARCH_COMMON_PARAMS, keyword, page },
      await getHeader(),
    );
    ...
    for (const item of result['data']?.['result'] ?? []) {
      ...
      items.push({
        ...
        cover: '',
        platform: 'bilibili',
        liveStatus: item['is_live'] ? LiveStatus.live : LiveStatus.offline,
      });
    }
    return items;
  }
  ...
  async getRoomDetail(roomId: string): Promise<LiveRoomJson> {
    ...
    let danmakuData: Record<string, unknown>;
    try {
  ...
  async getPlayQualites(roomId: string): Promise<LivePlayQualityJson[]> {
    const result = await getJson(
      ...
      { room_id: roomId, protocol: '0,1', format: '0,1,2', codec: '0,1', platform: 'html5', dolby: 5 },
      await getHeader(),
    );
  ...
  async getPlayUrls(roomId: string, quality: string): Promise<string[]> {
    ...
    const entries: { url: string; codec: string; host: string; format: string }[] = [];
    ...
    const rank = (e: { url: string; codec: string; format: string }): number => {
      ...
      if (e.codec !== '' && e.codec !== preferCodec) return 1;
      return 0;
    };
  ...
  async sendDanmaku(roomId: string, message: string): Promise<[boolean, string]> {
    try {
      ...
      const res = await upstream(`${sendUrl}?${qs}`, {
        method: 'POST',
...
registerSite('bilibili', new BiliBiliSite());
