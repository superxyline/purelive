...
const K_BASE_HEADERS: Record<string, string> = {
  'user-agent':
...
let sessionCookie = '';
const sessionCookieObj = new Map<string, string>();
let sessionBootstrap: Promise<void> | null = null;
let sessionUpdatedAt = 0;
const SESSION_LIFETIME_MS = 30 * 60 * 1000;

function effectiveCookie(): string {
  const configured = getCookie('kuaishou').trim();
...
async function captureSessionCookie(url: string): Promise<void> {
  const res = await upstream(url, { headers: K_BASE_HEADERS });
  const pairs: string[] = [];
...
function misc2dic(did: string): Record<string, unknown> {
...
async function registerDid(): Promise<unknown> {
  const did = sessionCookieObj.get('did');
  if (did == null || did === '') return null;
  return upstreamJson(
    'https://log-sdk.ksapisrv.com/rest/wd/common/log/collect/misc2?v=3.9.49&kpn=KS_GAME_LIVE_PC',
    { method: 'POST', headers: K_BASE_HEADERS, body: JSON.stringify(misc2dic(did)) },
  );
}

async function ensureSession(url: string): Promise<void> {
  if (getCookie('kuaishou').trim() !== '') return;
  const now = Date.now();
  if (sessionCookie !== '' && now - sessionUpdatedAt < SESSION_LIFETIME_MS) return;
...
function parseFansCount(counts: unknown): string {
  if (counts == null || typeof counts !== 'object') return '';
  const raw = String((counts as Record<string, unknown>)['fan'] ?? '').trim();
  if (raw === '') return '';
...
function coverOf(poster: unknown): string {
  const text = String(poster ?? '');
  return isImage(text) ? text : `${text}.jpg`;
}

interface KuaishouRoomEntry {
  liveStreamId: string;
...
function roomFromEntry(entry: KuaishouRoomEntry, includePlaybackData: boolean): LiveRoomJson {
  const { liveStream, author, gameInfo, liveStreamId, live, description } = entry;
  return {
    ...emptyRoom('kuaishou', String(author['id'] ?? '')),
...
async function loadRoom(roomId: string, includePlaybackData: boolean, ensure: boolean): Promise<LiveRoomJson> {
  const url = `https://live.kuaishou.com/u/${encodeURIComponent(roomId)}`;
  if (ensure) await ensureSession(url);
  const res = await upstream(url, { headers: roomHeaders() });
  if (res.status >= 400) throw new Error(`kuaishou room page upstream ${res.status}`);
  const text = /window\.__INITIAL_STATE__=(.*?);/.exec(res.text)?.[1] ?? '';
  if (text === '') throw new Error('Kuaishou initial state is missing');
  const jsonObj = JSON.parse(text.split('undefined').join('null')) as Record<string, any>;
  ...
  return roomFromEntry(
    { liveStreamId, author, liveStream, gameInfo, description, live },
    includePlaybackData,
  );
}

/** Dart parsePlayQualities：房间页 {h264,hevc} 与列表 [descriptor] 双形态合并。 */
function parsePlayQualities(raw: unknown): LivePlayQualityJson[] {
  const descriptors = Array.isArray(raw) ? raw : [raw];
  const merged = new Map<string, { name: string; sort: number; urls: string[] }>();

  for (const rawDescriptor of descriptors) {
    if (rawDescriptor == null || typeof rawDescriptor !== 'object') continue;
    let descriptor: Record<string, any> = rawDescriptor;
    // 优先 AVC（兼容性），HEVC 仅兜底
  ...
  const qualities: LivePlayQualityJson[] = [...merged.values()].map((entry) => ({
    quality: entry.name,
    sort: entry.sort,
    data: entry.urls,
  }));
...
class KuaishouSite implements Site {
  async getCategories(_page: number, _pageSize: number): Promise<LiveCategoryJson[]> {
    ...
    for (const item of base) {
      const children: LiveCategoryJson['children'] = [];
      ...
      for (let page = 1; ; page++) {
        ...
        const result = await upstreamJson(
          `https://live.kuaishou.com/live_api/category/data?${qs.toString()}`,
          { headers: K_BASE_HEADERS },
        ).catch(() => null);
        const subs: any[] = result?.['data']?.['list'] ?? [];
  ...
  async getCategoryRooms(cateId: string, _typeName: string, page: number, _pageSize: number): Promise<LiveRoomJson[]> {
    ...
    const result = await upstreamJson(`${api}?${qs.toString()}`, { headers: K_BASE_HEADERS });
  ...
  async getRecommendRooms(_page: number, _pageSize: number): Promise<LiveRoomJson[]> {
    const result = await upstreamJson('https://live.kuaishou.com/live_api/home/list', {
      headers: K_BASE_HEADERS,
    });
    const items: LiveRoomJson[] = [];
    for (const item of result['data']?.['list'] ?? []) {
  ...
  async getLiveStatus(roomId: string): Promise<boolean> {
    ...
    return room.liveStatus === LiveStatus.live;
  }

  async searchRooms(_keyword: string, _page: number, _pageSize: number): Promise<LiveRoomJson[]> {
...
registerSite('kuaishou', new KuaishouSite());
export { parsePlayQualities, effectiveCookie as getKuaishouEffectiveCookie };
