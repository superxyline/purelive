...
export async function streamProxy(target: string, reply: FastifyReply): Promise<void> {
  ...
  const headers: Record<string, string> = { referer: `${parsed.protocol}//${parsed.hostname}/`, origin: `${parsed.protocol}//${parsed.hostname}` };
  const range = reply.request.headers.range;
  if (typeof range === 'string') headers.range = range;

  try {
    const res = await upstream(target, { headers, timeoutMs: 30_000 });
    ...
    if (res.headers['accept-ranges']) reply.header('accept-ranges', res.headers['accept-ranges']);
