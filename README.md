...
- **多平台关注同步**：登录 B站/斗鱼/虎牙后，可在账号页或 Cookie 设置页一键把平台关注的主播同步到应用收藏（按平台+房间号自动去重，只增不删）
...
- **新增多平台关注同步**：账号页与 B站/斗鱼/虎牙 Cookie 设置页新增"同步关注主播"入口，登录后可一键把平台关注的主播同步到应用收藏
  ...
  - 同步自动去重（平台+房间号）、只增不删，完成后汇总新增/已有/跳过数量
...
本纯净版（2.1.0）的功能裁剪、安全加固、代码优化与版本迭代，均由 **Codex（AI 编程助手）与 DeepSeek（AI 模型）** 协作完成，包括但不限于：
