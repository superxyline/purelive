...
Future<void> main() async {
  ...
  ws.add(ser('type@=loginreq/roomid@=$roomId/devid@=$devid/rt@=$rt/ver@=21952015/vk@=0/ct@=1/uid@=$uid/stk@=$stk/'));
  await Future.delayed(Duration(seconds: 2));
  if (!gotLogin) { print('login fail'); await ws.close(); return; }

  ws.add(ser('type@=joingroup/rid@=$roomId/gid@=-9999/'));
  await Future.delayed(Duration(seconds: 1));

  final cst = DateTime.now().millisecondsSinceEpoch.toString();
  ws.add(ser('type@=chatmessage/roomid@=$roomId/content@=$msg/col@=0/pt@=0/ct@=$cst/sn@=0/ss@=0/uid@=$uid/nn@=PureLive/txt@=$msg/level@=1/dms@=5/cst@=$cst/'));
  print('>>> 已发送弹幕');
