...
class DouyinUtils {
  ...
  /// 对已经构造好的完整 URL（含 query）原位追加 a-bogus 签名。
  /// 纯 Dart 实现，替代旧的 flutter_js `DouyinSign.getAbogusUrl`。
  static String signFullUrl(String url) {
    final parsedUrl = Uri.parse(url);
    if (parsedUrl.query.isEmpty) return url;
    final abogus = ABogus(userAgent: DouyinRequestParams.kDefaultUserAgent);
    final signedQueryStr = abogus.generateAbogus(parsedUrl.query).first;
    return parsedUrl.replace(query: signedQueryStr).toString();
  }
