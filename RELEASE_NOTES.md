...
在平板上复现并抓了 mpv 内核属性，结论很明确：

- 黑屏时音频正常、弹幕正常，画面区域是纯黑，**没有任何报错提示**（所以既有的"出错后换线"逻辑不会触发）。
- 出问题的是 B站的 **HEVC + flv 线路**（`suffix=prohevc`）：mpv 打开后 `video-params/w`、`video-params/h`、`pixelformat` **一直为空**，也就是内核根本没能从这条流里解析出视频轨，只有音频轨；同时 `demuxer-cache-duration` 还会报出 632 秒这类异常值。
- 换成 HLS（`.m3u8`）线路则一切正常：`video-params/w=1920`、`pixelformat=nv12`、`estimated-vf-fps=60`。
- 而 B站的线路列表是 flv 在前、HLS/ts/fmp4 在后，加上"HEVC 优先"设置，默认就会先落到这两条坏线路上——这正是"打开就黑屏、手动切线路才出画面"的原因。

## 修复

1. **取流排序**：B站取流展开线路时，把 **HEVC 且格式为 flv** 的线路排到 ts/fmp4/HLS 之后（仍保留在列表里，其它线路都不可用时还能兜底）。这样默认就会选到能出画的线路。实测修复后只开一次线路：`format=HLS codec=hevc host=d1--cn-gotcha104`，mpv 在第 6 秒即达到 60fps 出画。
2. **"有声音没画面"看门狗**：开播 7 秒后若仍在播放、却始终没有有效的视频宽高，就按解码失败处理，自动切换到下一条线路（动作与手动切线路一致），连续无效最多换 4 次后照旧上报错误。这样即使日后平台又出现"静默无画面"的线路，也不必再手动切。
...
本版按反馈**移除"从赛事点击跳转直播间"的功能**，赛事中心只保留赛程/赛果本身。

## 移除内容

- 比赛卡片不再可点：之前进行中的比赛点击卡片任意位置会跳转观赛直播间，现已去掉该点击行为。
- 去掉卡片中间的「观看直播·平台」跳转胶囊，以及无官方直播间时弹出的「选择观赛直播间」（斗鱼 6657 / 虎牙 123321）弹窗。
- 赛事提醒通知点击后统一回首页，不再保留"跳转 CS 直播间"的分支（该分支原本也只是回首页，注释与实现一直不符）。
- 连带清理：模型里的 `livePlatform` / `liveRoomId` 字段与相应的接口解析、序列化，以及 `esports_select_live_room` / `esports_watch_live` 两个文案键。

## 保留内容

- 赛事中心本身不受影响：赛程/赛果、赛事关注与星标、筛选、热度排序、秒开缓存、LIVE 与比分展示均照旧。
...
本版修两个使用中反馈的问题，并按反馈调整了两处界面。

## 控制条：去掉重复的关注爱心

- 直播间顶部标题栏（直播名旁）已有关注爱心，底部控制条里那个是重复的，已移除；关注/取关仍可在顶部爱心或房间卡片上操作。

## 遮挡块：移到右上角菜单，支持三个

- 入口从控制条按钮改为直播间**右上角菜单**里的「遮挡块 1 / 2 / 3」，菜单项右侧显示当前是「已开启 / 已关闭」。
- 每个直播间可分别设置**最多 3 个**遮挡块，用来同时遮住画面上多处广告；三个槽位默认位置纵向错开，不会叠在一起。
- 每个遮挡块的开关与位置都单独记住：关掉只隐藏、位置保留，下次打开原样恢复；只有某个槽位从没设过时才用该槽位的默认位置。
- 旧版本的单遮挡块数据仍按槽位 1 读取，已调好的位置不会丢。

## 遮挡块：关闭后记住位置

- 之前关掉遮挡块会连同位置一起丢掉，再打开又回到默认位置，得重新拖一遍。现在把"是否显示"和"位置尺寸"分开保存：关闭只隐藏，位置保留，下次打开原样恢复。
- 关闭后再打开不再重复弹"如何调整"的提示（只有首次设置时提示一次）。

## 双指缩放还原按钮跟随控制条显隐

- 之前双指缩放画面后，即使控制条已自动隐藏，"一键还原画面"按钮仍会以半透明状态留在屏幕上。现在它完全跟随控制条：控制条隐藏时一并消失，点一下画面控制条回来时再一起出现（与顶部/底部控制栏的显隐时机一致）。

## 其他

- 版本号提升至 `2.1.3+4015`（`2.1.3+4014` 为本版早期的构建号，功能相同）。

---

# Pure Live v2.1.2

本版聚焦"关注房间多时冷启动首页刷新很慢"的问题：把批量刷新改成只取卡片需要的字段、提高并发、分批增量刷新界面，并修掉一处会让卡片被清空的数据损坏。

## 刷新慢的原因（定位结论）

- 关注列表批量刷新走的是**完整**房间详情，而各平台的完整详情会顺带发一堆"只有进房间才需要"的请求：B站每房间约 4 次（弹幕服务器发现 `getDanmuInfo`、取 buvid 的 `spi`、只为开播时间的 `room_init`、粉丝数为空时抓房间页），斗鱼 3 次（弹幕签名 `homeH5Enc`、主播粉丝接口），虎牙 2 次（抓粉丝的房间页），抖音 2 次（补粉丝/开播时间的 reflow），快手在没有配置 cookie 时还要每个房间重做一次匿名会话引导与设备注册。
- 并发数默认只有 **2**，且批次之间是串行 `await`，N 个房间要跑 ⌈N/2⌉ 个来回。
- 界面只在**所有批次跑完**后才更新一次，中途不刷新；刷新进度条此前只在桌面端显示，手机端全程没有任何提示。三者叠加就是"长时间停留在上次打开的首页状态"。
- 另外 `Sites.of()` 每次都会新建站点实例，导致 B站 buvid、快手匿名会话这类实例级缓存被逐个房间击穿。

## 本次改动

- 新增轻量取数模式（`getRoomDetail(light: true)`），只有收藏列表批量刷新会用它：
  - B站：跳过弹幕服务器发现（改给一份连接时再补凭据的默认参数，弹幕功能不受影响）、跳过粉丝数房间页抓取；保留 `getInfoByRoom` 与 `room_init`（角标需要开播时间）。
  - 斗鱼：跳过弹幕签名与粉丝接口，只发一次 `betard`。
  - 虎牙：跳过抓粉丝的房间页。
  - 抖音：卡片不显示粉丝数，enter 已带 `create_time` 时不再补 reflow（角标仍正常）。
  - 快手：不附带播放地址（卡片用不到，也避免把它写进收藏持久化）。
  - 安全前提：弹幕与播放流始终使用**进入房间后重新拉取的完整详情**（`onInitPlayerState` → `getRoomDetail` → `connectRoom(liveRoom)`），轻量详情只交给卡片渲染。
- B站 buvid、快手匿名会话与设备注册结果改为进程级共享，不再每个房间重来一次。
- 批量刷新并发默认值 2 → **6**（没改过该设置的老用户一次性迁移到新默认，手动调整过的不动）。
- 每批房间就绪后即把结果推给界面（约 400ms 节流），不再等全部刷完；刷新期间手机端也会显示顶部进度条。
- 新增刷新防重入：启动刷新、页签双击刷新与下拉刷新叠加时不再重复打接口。
- 修掉数据损坏：详情请求失败/被风控降级时平台返回的是只带 roomId 的空房间对象，原先会拿它覆盖缓存并落盘（标题、封面、开播时间被清空，下次冷启动直接显示空卡片）。现在合并前先判断卡片数据是否可用，不可用则整条保留旧数据。
...
本版为版本号提升：`2.1.1+4012`。功能与 v2.1.0（build 4011）一致——即画面模糊遮挡块（按直播间记忆）、互动体验中的全屏浮层透明度调节、抖音开播时长角标修复，以及此前的五平台粉丝数、B站解码失败自动换线路、观看统计礼物等功能，详见下方各段落。

---

# Pure Live v2.1.0（build 4011）

本构建在 4010 的基础上新增：互动体验里的全屏浮层透明度调节、抖音开播时长角标修复，以及画面上可自由摆放的模糊遮挡块。

## 画面遮挡块（新功能）

- 控制栏静音按钮旁新增「遮挡块」按钮：点击在当前直播间画面上放一个模糊框，用来遮住固定位置的广告（如主播挂的角标广告）；再点一次按钮即移除该直播间的遮挡块。
- 遮住的内容是**模糊**而不是盖一块死色，因此既挡住广告又不会在画面上留一块突兀的色块。
- 层级刻意放在弹幕层**之下**：弹幕始终绘制在遮挡块上面，不会被遮住。
- 遮挡块本体整体 `IgnorePointer`，指针事件继续下落到播放器手势层，所以滑动调亮度/音量、双击全屏、点击弹幕全部照常；只有拖动条与缩放手柄接收手势。
- 手柄（顶部拖动条移动、右下角手柄缩放）只在控制条显示时出现，平时完全不占用手势，也不会误触。
- 位置与大小按 `平台|房间号` **分别记住**，比例化存储（0~1），因此全屏与窗口模式、不同分辨率下都落在同一相对位置；换直播间互不影响，各房间的遮挡块独立开关。数据随备份 / 跨端传输同步。

## 互动体验

- 设置入口「本地互动体验」更名为「互动体验」。
- 新增「全屏浮层透明度」分组，两个滑动条分别调节全屏**醒目留言（SC）**与全屏**礼物卡片**的不透明度（10%~100%），默认 100%（即原有观感）。该分组与本地互动开关无关，始终可用。

## 抖音开播时长角标

- 修复抖音直播间卡片不显示已开播时长角标的问题。根因：抖音的房间页 HTML（`roomStore.roomInfo.room`）**没有 `create_time` 字段**（实测），而 web enter 接口的 room 对象也可能不带，代码里只从后者取值；一旦走到房间页回退路径，开播时间就为空，角标因此不显示。
- 现在改为从 `reflow/info` 的 `data.room.create_time` 兜底（秒级时间戳，实测有效），并与粉丝数共用同一次请求，不增加额外网络开销。

---

# Pure Live v2.1.0（build 4010）

本版本主题是"直播间信息补全 + 播放可靠性"：为五个平台的直播间信息补齐主播粉丝数，把 B站"只有声音没有画面"的解码失败做成自动换线路恢复，弹幕设置新增按直播间生效的礼物卡片开关，观看统计开始统计观看期间收到的礼物及其价值。

## 直播间粉丝数（五平台）

- B站：`getInfoByRoom` 的粉丝数字段是 `anchor_info.relation_info.attention`（单数），此前按搜索接口的复数 `attentions` 取值，取到的一直是空串，"有数据才显示"的逻辑又把它静默隐藏，所以该行长期不出现。现已修正，并新增房间页 HTML 抓取兜底（内嵌同一字段），接口风控缺失 `relation_info` 时仍能拿到。真机接口实测：房间 6 = 16872769、21452505 = 1114426。
- 虎牙：`mp profileRoom` 接口登录态也不返回粉丝字段（profileInfo 仅 8 个字段），改从房间页 HTML 的 `TT_PROFILE_INFO.fans` 提取；详情与房间页并行请求，WebView 回退路径同样提取（实测 CSBOY = 1980721）。
- 斗鱼：`betard` 接口整份响应里没有粉丝数字段（只有 `fans_bn` 粉丝牌名），备用源 `open.douyucdn.cn` 的 `fans_num` 已失效、实测恒为 0。改用主播卡片接口 `wgapi/livenc/liveweb/getAnchorNewCard` 的 `data.roomInfo.fansNum`，与取流加密脚本请求并行发出，不增加进入房间的耗时（实测 yyf = 2878265、英雄联盟赛事 = 18470829）。
- 抖音：`web enter` 接口与房间页 HTML 都没有粉丝数（`follow_info` 只剩关注状态），唯一来源是 `reflow/info` 的 `data.room.owner.follow_info`。用短链/网页号进入时以房间号补一次该请求（与 `_getRoomDataByRoomId` 同一路径，无需登录 cookie）；以房间号进入时该响应本来就已获取，零额外请求。数字字段优先，因为 `follower_count_str` 有时是"515.6万"这类展示串。
- 快手：取作者对象的 `counts.fan`，它是带单位的展示串（如 `"152.7w"`），统一换算成数值后交给展示层格式化。
- 五平台口径统一：接口拿不到就留空、不显示该行，不填假数据。

## 播放可靠性

- 修复 B站"当前播放器解码失败"但声音正常、手动切一个线路画面才出来的问题。根因：同一清晰度下的多条线路编码/封装组合不同，首条可能视频轨解不出来；而播放器的自动换线此前只覆盖 `network` / `source` 两类错误，解码（codec）错误既不换线也不重试，于是停在"有声无画"。
- 现在解码失败同样先尝试切换下一条线路（即用户手动切线路所做的那件事），换线成功不再上报错误，既不弹提示也不进入错误状态；所有线路都失败时才照旧上报。
- 顺带修掉"能自动换线恢复却仍先弹一次提示"的行为（网络、源错误同理），并在换线条件中补上当前播放地址非空的判断。

## 弹幕设置

- 礼物效果新增「弹幕列表礼物卡片（仅本直播间）」开关，默认打开，按 `平台|房间号` 单独保存：关掉后该直播间的弹幕列表不再渲染礼物卡片，其它直播间不受影响，重新打开立即恢复（消息本身不从列表中移除）。该配置随备份 / 跨端传输一并同步。
- 「醒目留言」设置分组只在 B站 直播间显示：该类消息只有 B站 会产生，其它平台显示该开关没有意义。

## 观看统计

- 在原有时长统计基础上，统计观看期间各直播间收到的礼物：总件数、礼物价值与主播到手估算，最常观看的房间行也会显示该房间的礼物数与价值。
- 计价口径集中在一处并按平台区分：B站按实付金瓜子（`total_coin` 优先，1000 金瓜子 = 1 元，银瓜子属免费礼物只计件数不计价值）；虎牙按实付总额（单位是 1/100 虎牙币，1 虎牙币 ≈ 1 元）；抖音 `diamondCount` 是单价（1 元 ≈ 10 钻石），需乘以数量。斗鱼协议不提供价格、快手匿名弹幕通道不下发礼物消息，这两家只统计件数。
- 抖音的 `repeatCount` 是连击**累计值**，按增量累计，避免一次 n 连击被算成 1+2+…+n 件而放大数倍。
- 本地互动礼物是"自己发的"、不属于收到，不参与统计。
- **主播到手比例（默认按 50%）与币值折算均为估算，不是平台官方数据**，界面已明确标注"估算"。
